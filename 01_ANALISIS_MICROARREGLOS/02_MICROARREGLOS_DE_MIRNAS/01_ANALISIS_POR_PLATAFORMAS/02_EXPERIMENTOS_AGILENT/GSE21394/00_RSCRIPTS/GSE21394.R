...
pdf("./04_ANALISIS_DE_CALIDAD/BP_RAWDATA_GSE21394.pdf")
boxplot(project$E,
        main="Boxplot of log2-intensitites for the Raw data",
        xlab="", ylab=bquote(~Log[2]~expression),
        names= targetinfo$Filename,
        col = c("darkslategray3", "darkslategray3","darkslategray3","darkslategray3","darkslategray3","darkslategray3","brown1",
...
pdf("./04_ANALISIS_DE_CALIDAD/BP_BC_GSE21394.pdf")
...
pdf("./04_ANALISIS_DE_CALIDAD/BP_NORM_DATA_GSE21394.pdf")
...
pdf("./04_ANALISIS_DE_CALIDAD/PCA_GSE21394.pdf")
ggplot(dataGG, aes(PC1, PC2, label=row.names(dataGG))) +
  geom_text(size=1)+geom_point(aes(colour = targetinfo$Group)) +
  ggtitle("GSE21394 | PCA plot of the log-transformed norm expression data") +
...
#Obtenemos los datos de anotacion del  project.NormData
...
FinalTable <- cbind(Annot3[2:3], table_CD, Annot3[4:18])

write.table(FinalTable, file="./05_TABLA_DE_EXPRESION_DIFERENCIAL/DEG_GSE21394.txt", sep="\t",
...
pdf("./06_GRAFICOS_DE_EXPRESION_DIFERENCIAL/Volcano_GSE21394.pdf")
EnhancedVolcano(FinalTable,lab = FinalTable$GeneName,
                ...
                legendLabels = c("NS", expression(LogFC), "adj.P.Val", expression(adj.P.Val ~ and
                                                                                  ~ logFC)),
                legendLabSize = 8,
                title = "     GSE21394 | Volcano plot of differential expression",
                titleLabSize = 13,
                subtitleLabSize = 9,
                captionLabSize = 9,
                border = 'full',
...
write.table(data_filtered, file="./07_RESULTADOS/DEG_FILTER_GSE21394.txt", sep="\t", row.names=F, col.names=T, quote=F)
...
dataZ <- t(apply(data_filtered[,9:23], 1, cal_z_score))
data_filtered2 <- cbind(data_filtered[,1:8], dataZ[,1:6], dataZ[,7:15])
data_filtered3 <- data_filtered2 %>% drop_na

#Seleccionar color
col_fun <- colorRamp2(seq(min(data_filtered3[,9:23]), max(data_filtered3[,9:23]), length = 3), c("#0000ff", "white", "#fb0007"))
col_fun
...
#Heatmap sin genes
pdf("./06_GRAFICOS_DE_EXPRESION_DIFERENCIAL/Heatmap_GSE21394.pdf")
Heatmap(as.matrix(data_filtered3[,9:23]),
        name = "Z-Score", column_title = "GSE21394 | Differential gene expression heatmap",  column_title_gp = gpar(fontsize = 13, fontface = "bold"),
        col = rwb,
        column_order = order(as.numeric(gsub("column", "", colnames(data_filtered3[,9:23])))),
