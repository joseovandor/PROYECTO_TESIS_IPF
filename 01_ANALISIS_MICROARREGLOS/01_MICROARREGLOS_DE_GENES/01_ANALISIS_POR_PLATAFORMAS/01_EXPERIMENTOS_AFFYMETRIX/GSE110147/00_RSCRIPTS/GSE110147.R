#####################################################################
#Titulo: Pipeline para Affymetrix Human Gene 1.0 ST Array
...
untar("./01_DATOS/GSE110147_RAW.tar", exdir = "./01_DATOS/CEL")
...
arrayQualityMetrics(expressionset = raw_data[, 1:30],
...
pdf("./04_ANALISIS_DE_CALIDAD/PCA_GSE110147.pdf")
ggplot(dataGG, aes(PC1, PC2, label=row.names(dataGG))) +
  geom_point(aes( colour = Group))+
  geom_text(size=1)+
  ggtitle("GSE110147 | PCA plot of the log-transformed norm expression data") +
...
Annot <- AnnotationDbi::select(hugene10sttranscriptcluster.db,
...
fit<-lmFit(Annot_final[5:34],design)
...
FinalTable = data.frame(Annot_final[,1:4], table_CD, Annot_final[,5:34])

write.table(FinalTable, file="./05_TABLA_DE_EXPRESION_DIFERENCIAL/DEG_GSE32537.txt", sep="\t",
...
keyvals <- ifelse(FinalTable$logFC <= -1.5 & FinalTable$adj.P.Val <0.05,
                  '#0000ff',  ifelse(FinalTable$logFC >=1.5 & FinalTable$adj.P.Val <0.01,
                                     '#fb0007', '#e3deeb'))
...
pdf("./06_GRAFICOS_DE_EXPRESION_DIFERENCIAL/Volcano_GSE110147.pdf")
EnhancedVolcano(FinalTable,lab = FinalTable$SYMBOL,
                ...
                legendLabels = c("NS", expression(LogFC), "adj.P.Val", expression(adj.P.Val ~ and
                                                                                  ~ logFC)),
                legendLabSize = 8,
                title = "     GSE110147 | Volcano plot of differential expression",
                titleLabSize = 13,
                subtitleLabSize = 9,
                captionLabSize = 9,
                border = 'full',
...
#Filtrado de datos
data_filtered <- FinalTable %>% filter(adj.P.Val < 0.05 & (logFC > 1.5 | logFC < -1.5 ))
...
write.table(data_filtered, file="./07_RESULTADOS/DEG_FILTER_GSE110147.txt", sep="\t", row.names=F, col.names=T, quote=F)
...
dataZ <- t(apply(data_filtered[,11:40], 1, cal_z_score))
data_filtered2 <- cbind(data_filtered[,1:10], dataZ[,20:30], dataZ[,1:19])
data_filtered3 <- data_filtered2 %>% drop_na

#Seleccionar color
col_fun <- colorRamp2(seq(min(data_filtered3[,11:40]), max(data_filtered3[,11:40]), length = 3), c("#0000ff", "white", "#fb0007"))
col_fun
...
#Heatmap sin genes
pdf("./06_GRAFICOS_DE_EXPRESION_DIFERENCIAL/Heatmap_GSE110147.pdf")
Heatmap(as.matrix(data_filtered3[,11:40]),
        name = "Z-Score", column_title = "GSE110147 | Differential gene expression heatmap",  column_title_gp = gpar(fontsize = 13, fontface = "bold"),
        col = rwb,
        column_order = order(as.numeric(gsub("column", "", colnames(data_filtered3[,11:40])))),
