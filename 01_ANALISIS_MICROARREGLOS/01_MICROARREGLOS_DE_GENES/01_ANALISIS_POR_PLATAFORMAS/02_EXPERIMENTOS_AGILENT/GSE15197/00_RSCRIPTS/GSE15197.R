...
pdf("./04_ANALISIS_DE_CALIDAD/BP_RAWDATA_GSE15197.pdf")
boxplot(project$E,
        ...
        col = c("darkslategray3","darkslategray3","darkslategray3","darkslategray3"
                ...
                "darkslategray3","darkslategray3","darkslategray3",
                "darkslategray3","darkslategray3","brown1"
                ,"brown1","brown1","brown1","brown1","brown1","brown1"),
        las=2,
        boxwex = 0.6,
        staplewex = 0.6,
        outline=F)
...
pdf("./04_ANALISIS_DE_CALIDAD/BP_BC_GSE15197.pdf")
boxplot(project.bgc$E,
        main="Boxplot of log2-intensitites for the raw data with Background corecction",
        xlab="", ylab=bquote(~Log[2]~expression),
        names= targetinfo$Filename,
        col = c("darkslategray3","darkslategray3","darkslategray3","darkslategray3"
                ,"darkslategray3","darkslategray3","darkslategray3",
...
pdf("./04_ANALISIS_DE_CALIDAD/BP_NORM_DATA_GSE15197.pdf")
boxplot(project.NormData$E,
        main="Boxplot of log2-intensitites for normData",
        names= targetinfo$Filename,
        xlab="", ylab=bquote(~Log[2]~expression),
        col = c("darkslategray3","darkslategray3","darkslategray3","darkslategray3"
                ,"darkslategray3","darkslategray3","darkslategray3",
...
pdf("./04_ANALISIS_DE_CALIDAD/PCA_GSE15197.pdf")
ggplot(dataGG, aes(PC1, PC2, label=row.names(dataGG))) +
  geom_text(size=1)+geom_point(aes(colour = targetinfo$Group)) +
  ggtitle("GSE15197 | PCA plot of the log-transformed norm expression data") +
...
Annot2 <- Annot1[1:45015, c(4, 5, 6, 7, 8)]
...
FinalTable = data.frame(Annot3[,2:5], table_CD, Annot3[,6:24])

write.table(FinalTable, file="./05_TABLA_DE_EXPRESION_DIFERENCIAL/DEG_GSE15197.txt", sep="\t",
...
pdf("./06_GRAFICOS_DE_EXPRESION_DIFERENCIAL/Volcano_GSE15197.pdf")
EnhancedVolcano(FinalTable,lab = FinalTable$GeneName,
                ...
                legendLabels = c("NS", expression(LogFC), "adj.P.Val", expression(adj.P.Val ~ and
                                                                                  ~ logFC)),
                legendLabSize = 8,
                title = "     GSE15197 | Volcano plot of differential expression",
                titleLabSize = 13,
                subtitleLabSize = 9,
                captionLabSize = 9,
                border = 'full',
...
write.table(data_filtered, file="./07_RESULTADOS/DEG_FILTER_GSE15197.txt", sep="\t", row.names=F, col.names=T, quote=F)
...
dataZ <- t(apply(data_filtered[,11:29], 1, cal_z_score))
data_filtered2 <- cbind(data_filtered[,1:9], dataZ[,1:12], dataZ[,13:19])
data_filtered3 <- data_filtered2 %>% drop_na

#Seleccionar color
col_fun <- colorRamp2(seq(min(data_filtered3[,10:21]), max(data_filtered3[,10:28]), length = 3), c("#0000ff", "white", "#fb0007"))
col_fun
...
#Heatmap sin genes
pdf("./06_GRAFICOS_DE_EXPRESION_DIFERENCIAL/Heatmap_GSE15197.pdf")
Heatmap(as.matrix(data_filtered3[,10:28]),
        name = "Z-Score", column_title = "GSE15197 | Differential gene expression heatmap",  column_title_gp = gpar(fontsize = 13, fontface = "bold"),
        col = col_fun,
        column_order = order(as.numeric(gsub("column", "", colnames(data_filtered3[,10:28])))),
